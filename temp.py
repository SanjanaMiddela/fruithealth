...
def fruit(data):
    ...
    '''predicted_class = class_names[np.argmax(score_apple)]
    if predicted_class in class_names:
        if class_names[np.argmax(score_apple)][:6] == "rotten":
            return ("This", class_names[np.argmax(score_apple)][6:], " is {:.2f}".format(100 - (100 * np.max(score_apple))), "% healthy")
        else:
            return ("This", predicted_class, " is {:.2f}".format(100 * np.max(score_apple)), "% healthy")
    else:
        return ("Not a fruit. Predicted class:", predicted_class)'''
